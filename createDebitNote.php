<?php 
	...
	if(isset($_POST['Submit'])){
		$invoiceID = $_POST['debitNoteInvoiceID'];
		$debitNoteNo = makeSafe($_POST['debitNoteNo']);
		...
		$sqlCreateDebitNote = "INSERT INTO tbdebitnote(debitNote_id, debitNote_no, debitNote_date, debitNote_invoiceID, 
		debitNote_title, debitNote_content, debitNote_from, debitNote_terms, debitNote_attention, debitNote_email, 
		debitNote_subTotal, debitNote_taxTotal, debitNote_grandTotal, debitNote_discountTotal, debitNote_totalAfterDiscount, 
		debitNote_roundAmount, debitNote_grandTotalRound, debitNote_roundStatus) 
		VALUES(NULL, '$debitNoteNo', '$dateDebitNote', '$invoiceID', '$debitNoteTitle', '$debitNoteContent', '$debitNoteForm', 
		'$debitNoteTerms', '$debitNoteAttention', '$debitNoteEmail', '$subTotal', 
		'$taxTotal', '$grandTotal', '$discountTotal', '$totalAfterDiscount', '$roundAmount', '$groundTotalRound', '$roundStatus')";
		
		mysqli_query($connection, $sqlCreateDebitNote) or die("error in create debit note query");
		
		//create the debit note detail records
		$debitNoteID = mysqli_insert_id($connection);
		...
		VALUES(NULL, '$customerID', '$dateDebitNote', '$debitNoteNo', 'DN', '$debitNoteID', 'DEBIT NOTE', '$groundTotalRound')";
		...
		VALUES(NULL, 2, '$dateDebitNote', '$debitNoteNo', 'DN', '$debitNoteID', '$debitNoteCustomer', '$groundTotalRound', 0)";	
		...
		account4_documentType, account4_documentTypeID, account4_description, account4_debit, account4_credit) 
		VALUES(NULL, 4, '$dateDebitNote', '$debitNoteNo', 'DN', '$debitNoteID', '$debitNoteCustomer', 0, '$totalAfterDiscount')";
		...
		if($roundAmount == 0.00){	
			...
			//negative, Credit Expense account, gain to company since DN
		...
		$sqlUpdateInvoiceDebitNote = "UPDATE tbinvoice SET invoice_debitNote = (invoice_debitNote + $groundTotalRound) 
		WHERE invoice_id = '$invoiceID'";
		mysqli_query($connection, $sqlUpdateInvoiceDebitNote) or die("error in update invoice credit note query");		
	...
	//get the MAX id from tbdebitnote
	$sqlMaxID = "SELECT debitNote_no FROM tbdebitnote ORDER BY debitNote_id DESC LIMIT 0,1";
	$resultMax = mysqli_query($connection, $sqlMaxID) or die("error in max ID query");
	$strLastDebitNoteNo = "";
	if(mysqli_num_rows($resultMax) > 0){
		$rowLastDebitNoteNo = mysqli_fetch_row($resultMax);
		$strLastDebitNoteNo = $rowLastDebitNoteNo[0];
	}	
...
function check(it) {
	...
	function CheckDebitNoteNo(){		
		var checkEmptyInvoiceNo = $("#debitNoteNo").val();
		if(checkEmptyInvoiceNo===""){
			$("#info").html("<img border='0' src='images/Delete.ico'>");
			document.getElementById("bt").disabled = true;
		}else{			
			var data = "debitNoteNo="+$("#debitNoteNo").val();
			
			$.ajax({			 
				type : 'POST',
				url : 'checkExistingDebitNote.php',
				data : data,
				dataType : 'json',
				success : function(r)
				{
	...
	function ResubmitForm(g){
		if(g==0){					
			...
			var u = confirm("Do you want to Create Debit Note?");
			if(u==true){
				$('#debitNoteForm').attr('onsubmit', 'return true');
			jQuery("#bt").trigger('click'); //auto submit by clicking the submit button id
			}		
		
		}else{
			alert("This Debit Note No already exist in Database!");
		}		
...
<p></p>
<form id="debitNoteForm" action="createDebitNote.php" method="POST" onsubmit="return CheckAndSubmit()">
<table width="1200px" cellpadding="0" border="1" class="mystyle">
<tr><td colspan="4" align="center"><h1>Create Debit Note</h1></td><td></td></tr>
<!--<tr><td></td><td></td><td>&nbsp;</td><td></td><td></td></tr>-->
<tr><td>To</td><td><input type="text" name="debitNoteCustomer" size="45" maxlength="100" value="<?php echo $rowCustomerInfo[0];?>" readonly></td><td></td><td>Debit Note No</td><td><input type="text" name="debitNoteNo" id="debitNoteNo" size="12" maxlength="30" value="<?php echo $strLastDebitNoteNo;?>" onchange="CheckDebitNoteNo()" autofocus><span id="info"><img border="0" src="images/Terminate.png"></span></td></tr>

<tr><td>Attention</td><td><input type="text" name="debitNoteAttention" size="40" maxlength="100" value="<?php echo $rowCustomerInfo[3];?>"></td><td></td><td>Date</td><td><input id="demo2" type="text" name="debitNoteDate" size="10" value="<?php echo date('d/m/Y');?>" readonly>&nbsp;</td></tr>
<tr><td>Address</td><td><?php echo $rowCustomerInfo[4];?></td><td></td><td>Invoice No</td><td><?php echo $rowInvoiceInfo[10];?></td></tr>
<tr><td>Telephone</td><td><?php echo $rowCustomerInfo[1]?></td><td></td><td>From</td><td><input type="text" name="debitNoteForm" size="20" maxlength="50"></td></tr>
<tr><td>Email</td><td><input type="text" name="debitNoteEmail" size="40" maxlength="100" value="<?php echo $rowCustomerInfo[2];?>"></td><td></td><td></td><td></td></tr>

<tr><td colspan="5" align="left"></td></tr>
<tr><td colspan="5" align="left">&nbsp;<input type="text" name="debitNoteTitle" maxlength="200" size="150" placeholder="Debit Note Title"></td></tr>
<tr><td colspan="5" align="left"><textarea name="debitNoteContent" id="debitNoteContent"><?php if($duplicateStatus==1){echo $rowInvoiceInfo[0];}?></textarea></td></tr>
</table>
...
<table width="1200px" cellpadding="0" border="1" class="mystyle">
<tr><td colspan="5" align="left"><textarea name="debitNoteTerms" id="debitNoteTerms"></textarea>
<input type="hidden" name="debitNoteInvoiceID" id="debitNoteInvoiceID" value="<?php echo $invoiceDuplicateID;?>">
...
<p><input type="submit" id="bt" name="Submit" value="Create Debit Note" <?php if($duplicateStatus==1){echo "disabled";}?>></p>
