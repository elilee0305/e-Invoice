<?php 
	...
	if(isset($_POST['Submit'])){
		$invoiceID = $_POST['creditNoteInvoiceID'];
		$creditNoteNo = makeSafe($_POST['creditNoteNo']);
		$dateCreditNote = convertMysqlDateFormat($_POST['creditNoteDate']);	
		$customerID = $_POST['idCustomer'];
		
		$creditNoteForm = makeSafe($_POST['creditNoteForm']);
		...
		$sqlCreateCreditNote = "INSERT INTO tbcreditnote(creditNote_id, creditNote_no, creditNote_date, creditNote_invoiceID, 
		creditNote_title, creditNote_content, creditNote_from, creditNote_terms, creditNote_attention, creditNote_email, 
		creditNote_subTotal, creditNote_taxTotal, creditNote_grandTotal, creditNote_discountTotal, creditNote_totalAfterDiscount, 
		creditNote_roundAmount, creditNote_grandTotalRound, creditNote_roundStatus) 
		VALUES(NULL, '$creditNoteNo', '$dateCreditNote', '$invoiceID', '$creditNoteTitle', '$creditNoteContent', '$creditNoteForm', 
		'$creditNoteTerms', '$creditNoteAttention', '$creditNoteEmail', '$subTotal', 
		'$taxTotal', '$grandTotal', '$discountTotal', '$totalAfterDiscount', '$roundAmount', '$groundTotalRound', '$roundStatus')";
		
		mysqli_query($connection, $sqlCreateCreditNote) or die("error in create credit note query");
		
		//create the credit note detail records
		$creditNoteID = mysqli_insert_id($connection);
		...
		VALUES(NULL, '$customerID', '$dateCreditNote', '$creditNoteNo', 'CN', '$creditNoteID', 'CREDIT NOTE', '$groundTotalRound')";
		...
		VALUES(NULL, 2, '$dateCreditNote', '$creditNoteNo', 'CN', '$creditNoteID', '$creditNoteCustomer', 0, '$groundTotalRound')";	
		...
		account4_documentType, account4_documentTypeID, account4_description, account4_debit, account4_credit) 
		VALUES(NULL, 4, '$dateCreditNote', '$creditNoteNo', 'CN', '$creditNoteID', '$creditNoteCustomer', '$totalAfterDiscount', 0)";
		...
		//Sales Tax Payable only if got sales tax
		if($taxTotal <> 0.00){
		...
		$sqlUpdateInvoiceCreditNote = "UPDATE tbinvoice SET invoice_creditNote = (invoice_creditNote + $groundTotalRound) 
	...
	//get the MAX id from tbcreditnote
	$sqlMaxID = "SELECT creditNote_no FROM tbcreditnote ORDER BY creditNote_id DESC LIMIT 0,1";
	$resultMax = mysqli_query($connection, $sqlMaxID) or die("error in max ID query");
	$strLastCreditNoteNo = "";
	if(mysqli_num_rows($resultMax) > 0){
		$rowLastCreditNoteNo = mysqli_fetch_row($resultMax);
		$strLastCreditNoteNo = $rowLastCreditNoteNo[0];
	}	
	...
	if(isset($_GET['idInvoiceDuplicate'])){
		...
		invoice_no FROM tbinvoice WHERE invoice_id = '$invoiceDuplicateID'";
...
function check(it) {
	...
	function CheckCreditNoteNo(){		
		var checkEmptyInvoiceNo = $("#creditNoteNo").val();
		if(checkEmptyInvoiceNo===""){
			$("#info").html("<img border='0' src='images/Delete.ico'>");
			document.getElementById("bt").disabled = true;
		}else{			
			var data = "creditNoteNo="+$("#creditNoteNo").val();
			
			$.ajax({			 
				type : 'POST',
				url : 'checkExistingCreditNote.php',
				data : data,
				dataType : 'json',
				success : function(r)
				{
	...
	function ResubmitForm(g){
		if(g==0){					
			...
			var u = confirm("Do you want to Create Credit Note?");
			if(u==true){
				$('#creditNoteForm').attr('onsubmit', 'return true');
			jQuery("#bt").trigger('click'); //auto submit by clicking the submit button id
			}		
		
		}else{
			alert("This Credit Note No already exist in Database!");
		}		
...
<main2>
  <nav class="floating-menu">
    <h3></h3>
    <a href="invoiceListCreditNote.php">Invoice List</a>
    
  </nav>
</main2>
<p></p>
<form id="creditNoteForm" action="createCreditNote.php" method="POST" onsubmit="return CheckAndSubmit()">
<table width="1200px" cellpadding="0" border="1" class="mystyle">
<tr><td colspan="4" align="center"><h1>Create Credit Note</h1></td><td></td></tr>
<!--<tr><td></td><td></td><td>&nbsp;</td><td></td><td></td></tr>-->
<tr><td>To</td><td><input type="text" name="creditNoteCustomer" size="45" maxlength="100" value="<?php echo $rowCustomerInfo[0];?>" readonly></td><td></td><td>Credit Note No</td><td><input type="text" name="creditNoteNo" id="creditNoteNo" size="12" maxlength="30" value="<?php echo $strLastCreditNoteNo;?>" onchange="CheckCreditNoteNo()" autofocus><span id="info"><img border="0" src="images/Terminate.png"></span></td></tr>

<tr><td>Attention</td><td><input type="text" name="creditNoteAttention" size="40" maxlength="100" value="<?php echo $rowCustomerInfo[3];?>"></td><td></td><td>Date</td><td><input id="demo2" type="text" name="creditNoteDate" size="10" value="<?php echo date('d/m/Y');?>" readonly>&nbsp;</td></tr>
<tr><td>Address</td><td><?php echo $rowCustomerInfo[4];?></td><td></td><td>Invoice No</td><td><?php echo $rowInvoiceInfo[10];?></td></tr>
<tr><td>Telephone</td><td><?php echo $rowCustomerInfo[1]?></td><td></td><td>From</td><td><input type="text" name="creditNoteForm" size="20" maxlength="50"></td></tr>
<tr><td>Email</td><td><input type="text" name="creditNoteEmail" size="40" maxlength="100" value="<?php echo $rowCustomerInfo[2];?>"></td><td></td><td></td><td></td></tr>

<tr><td colspan="5" align="left"></td></tr>
<tr><td colspan="5" align="left">&nbsp;<input type="text" name="creditNoteTitle" maxlength="200" size="150" placeholder="Credit Note Title"></td></tr>
<tr><td colspan="5" align="left"><textarea name="creditNoteContent" id="creditNoteContent"><?php if($duplicateStatus==1){echo $rowInvoiceInfo[0];}?></textarea></td></tr>
</table>
...
<table width="1200px" cellpadding="0" border="1" class="mystyle">
<tr><td colspan="5" align="left"><textarea name="creditNoteTerms" id="creditNoteTerms"></textarea>
<input type="hidden" name="creditNoteInvoiceID" id="creditNoteInvoiceID" value="<?php echo $invoiceDuplicateID;?>">
<input type="hidden" name="invoiceCheckbox" id="invoiceCheckbox" value="<?php echo $strInvoiceCheckbox?>">
<input type="hidden" name="idCustomer" id="idCustomer" value="<?php echo $customerID?>">
...
<p><input type="submit" id="bt" name="Submit" value="Create Credit Note" <?php if($duplicateStatus==1){echo "disabled";}?>></p>
