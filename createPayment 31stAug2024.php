		...
		if(mysqli_num_rows($resultCheckExistingPayment)>0){
			...
			if(empty($aInvoiceIDselected)){
				...
				for($i=0; $i < $countInvoiceIDselected; $i++){
					$invoiceAmount = 0.00;
					$invoicePaid = 0.00;
					$invoiceStatus = "NP"; //default not paid
					
					$invoiceIDvalue = $aInvoiceIDselected[$i];
					$invoiceActualTotalName = "invoiceActualTotal".$invoiceIDvalue;
					...
					if($invoiceAmount > 0.00){
						...
						invoice_paymentStatus = '$invoiceStatus' WHERE invoice_id = '$invoiceIDvalue'";
