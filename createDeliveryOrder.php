<?php 
	...
	if(isset($_POST['Submit'])){
		...
		deliveryOrder_roundAmount, deliveryOrder_grandTotalRound, deliveryOrder_roundStatus) 
		VALUES(NULL, '$deliveryOrderNo', '$dateDeliveryOrder', '$customerID', '$deliveryOrderTitle', '$deliveryOrderContent', '$deliveryOrderFrom', 
		'$deliveryOrderTerms', '$customerAttentionValue', '$customerEmailValue', '$subTotal', 
	...
	$sqlCompanyName = "SELECT company_name, company_deliveryOrderTerms FROM tbcompany WHERE company_id = 1";
	...
	$sqlMaxID = "SELECT deliveryOrder_no FROM tbdeliveryorder ORDER BY deliveryOrder_id DESC LIMIT 0,1";
	...
	//get the duplicate Quotation info only if idDeliveryOrderDuplicate parameter set
	$duplicateStatus = 0;
	$customerID = 0;
	if(isset($_GET['idDeliveryOrderDuplicate'])){
		$duplicateStatus = 1;
		$deliveryOrderDuplicateID = $_GET['idDeliveryOrderDuplicate'];
		...
		$sqlDeliveryOrderInfo = "SELECT deliveryOrder_content, deliveryOrder_terms, deliveryOrder_subTotal, deliveryOrder_taxTotal, deliveryOrder_grandTotal,
		deliveryOrder_discountTotal, deliveryOrder_totalAfterDiscount, deliveryOrder_roundAmount, deliveryOrder_grandTotalRound, deliveryOrder_roundStatus 
		FROM tbdeliveryorder WHERE deliveryOrder_id = '$deliveryOrderDuplicateID'";
		...
		WHERE deliveryOrderDetail_deliveryOrderID = '$deliveryOrderDuplicateID' ORDER BY deliveryOrderDetail_sortID ASC";
...
function check(it) {
	...
	function GetDuplicateInfo(){
		var checkDuplicateQuotationNo = $("#deliveryOrderDuplicate").val();
		var customerID = $("#customerIDvalue").val();
		
		if(checkDuplicateQuotationNo===""){
			//do nothing				
			deliveryOrderDuplicate.focus();
			
		}else{
			
			var data = "deliveryOrderNo="+$("#deliveryOrderDuplicate").val();
			...
			$.ajax({			 
				...
				success : function(r)
				{
					if(r=="0"){
						...
						window.location.href = "createDeliveryOrder.php?idCustomer="+customerIDvalue+"&idDeliveryOrderDuplicate="+quotationIDvalue;
	...
	function ResubmitForm(g){
		if(g==0){					
			...
			var u = confirm("Do you want to Create Delivery Order?");
			if(u==true){
...
// ARRAY FOR HEADER.
	...
	// EXTRACT AND SUBMIT TABLE DATA.//not using here because using PHP to create the delivery order details
    function sumbit() {
        var myTab = document.getElementById('empTable');
        var values = new Array();
...
<p></p>
<form id="deliveryOrderForm" action="createDeliveryOrder.php" method="POST" onsubmit="return CheckAndSubmit()">
<table width="1200px" cellpadding="0" border="1" class="mystyle">
<tr><td colspan="4" align="center"><h1>Create Delivery Order</h1></td><td></td></tr>
...
</td><td><input type="text" name="deliveryOrderDuplicate" id="deliveryOrderDuplicate" size="12" maxlength="30">&nbsp;<img src="images/duplicatefinal.png" onclick="GetDuplicateInfo()"><span id="duplicateInfo" style="color:blue;font-size:12px"></span></td></tr>
<!--<tr><td></td><td></td><td>&nbsp;</td><td></td><td></td></tr>-->
<tr><td>To</td><td><input type="customerNameValue" id="customerNameValue" size="60" value="<?php if(($duplicateStatus==1)&&($customerID <> 0)){echo $rowCustomerInfo[0];}?>" readonly></td><td></td><td>Delivery Order No</td><td><input type="text" name="deliveryOrderNo" id="deliveryOrderNo" size="12" maxlength="30" value="<?php echo $strLastQuotationNo;?>" onchange="CheckDeliveryOrderNo()" autofocus><span id="info"><img border="0" src="images/Terminate.png"></span></td></tr>

<tr><td>Attention</td><td><input type="text" name="customerAttentionValue" id="customerAttentionValue" size="40" maxlength="100" value="<?php if(($duplicateStatus==1)&&($customerID <> 0)){echo $rowCustomerInfo[3];}?>"></td><td></td><td>Date</td><td><input id="demo2" type="text" name="deliveryOrderDate" size="10" value="<?php echo date('d/m/Y');?>" readonly>&nbsp;</td></tr>
<tr><td>Address</td><td><input type="text" name="customerAddressValue" id="customerAddressValue" size="70" value="<?php if(($duplicateStatus==1)&&($customerID <> 0)){echo $rowCustomerInfo[4];}?>" readonly></td><td></td><td>From</td><td><input type="text" name="deliveryOrderFrom" size="20" maxlength="50"></td></tr>
...
<tr><td colspan="5" align="left">&nbsp;<input type="text" name="deliveryOrderTitle" maxlength="200" size="150" placeholder="Delivery Order Title"></td></tr>
<tr><td colspan="5" align="left"><textarea name="deliveryOrderContent" id="deliveryOrderContent"><?php if($duplicateStatus==1){echo $rowDeliveryOrderInfo[0];}?></textarea></td></tr>
</table>
<!--this area we put the delivery order details-->
	...
	if($duplicateStatus==1){		
		...
		if(mysqli_num_rows($resultDeliveryOrderDetailInfo) > 0){
			...
			while($rowDeliveryOrderDetailInfo = mysqli_fetch_row($resultDeliveryOrderDetailInfo)){
				...
				$strQuotationCheckbox2 = $rowDeliveryOrderDetailInfo[10];			
				if($numOfRows > 1){
					if($counter == 1){
						$strDeliveryOrderCheckbox = $strDeliveryOrderCheckbox.$strQuotationCheckbox2;
					}else{					
					...
					$strDeliveryOrderCheckbox = $strQuotationCheckbox2;
				}			
...
<?php
	if($duplicateStatus==1){	
		if(mysqli_num_rows($resultDeliveryOrderDetailInfo) > 0){
			...
			echo "<tr><td style='width:76%'></td><td style='width:15%'>Subtotal</td><td style='width:9%'><input type='text' id='subTotal' name='subTotal' class='styled4' value='$rowDeliveryOrderInfo[2]' readonly></td><tr>";
			
			echo "<tr><td style='width:76%'></td><td style='width:15%'>Discount Total</td><td style='width:9%'><input type='text' id='discountTotal' name='discountTotal' class='styled4' value='$rowDeliveryOrderInfo[5]' readonly></td><tr>";
			echo "<tr><td style='width:76%'></td><td style='width:15%'>Total After Discount</td><td style='width:9%'><input type='text' id='totalAfterDiscount' name='totalAfterDiscount' class='styled4' value='$rowDeliveryOrderInfo[6]' readonly></td><tr>";
			
			echo "<tr><td style='width:76%'></td><td style='width:15%'>Tax</td><td style='width:9%'><input type='text' id='taxTotal' name='taxTotal' class='styled4' value='$rowDeliveryOrderInfo[3]' readonly></td><tr>";
			echo "<tr><td style='width:76%'></td><td style='width:15%'>Total</td><td style='width:9%'><input type='text' id='grandTotal' name='grandTotal' class='styled4' value='$rowDeliveryOrderInfo[4]' readonly></td><tr>";
			echo "<tr><td style='width:76%'></td><td style='width:15%'>Round</td><td style='width:9%'><input type='text' id='roundAmount' name='roundAmount' class='styled4' value='$rowDeliveryOrderInfo[7]' readonly>";
			
			if($rowDeliveryOrderInfo[9] == 1){
			...
			echo "</td><tr>";
			echo "<tr><td style='width:76%'></td><td style='width:15%'>Rounded Total</td style='width:9%'><td><input type='text' id='groundTotalRound' name='groundTotalRound' class='styled4' value='$rowDeliveryOrderInfo[8]' readonly><input type='hidden' id='counterValue' value='$counter'></td><tr>";
...
<table width="1200px" cellpadding="0" border="1" class="mystyle">
<tr><td colspan="5" align="left"><textarea name="deliveryOrderTerms" id="deliveryOrderTerms"><?php if($duplicateStatus==1){echo $rowDeliveryOrderInfo[1];}else{echo $rowCompanyName[1];} ?></textarea></td></tr>
...
<p><input type="submit" id="bt" name="Submit" value="Create Delivery Order" <?php if($duplicateStatus==1){echo "disabled";}?>></p>
