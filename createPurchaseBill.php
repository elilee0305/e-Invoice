<?php 
	...
	if(isset($_POST['Submit'])){
		...
		$customerNameValue =  $_POST['customerNameValue'];
		...
		purchaseBill_roundAmount, purchaseBill_grandTotalRound, purchaseBill_roundStatus, purchaseBill_account3ID, purchaseBill_customerInvoiceNo) 
		VALUES(NULL, '$purchaseBillNo', '$datePurchaseBill', '$customerID', '$purchaseBillTitle', '$purchaseBillContent', '$purchaseBillFrom', 
		'$purchaseBillTerms', '$customerAttentionValue', '$customerEmailValue', '$subTotal', 
		'$taxTotal', '$grandTotal', '$discountTotal', '$totalAfterDiscount', '$roundAmount', '$groundTotalRound', '$roundStatus', 
		'$purchaseBillAccount3', '$purchaseBillCustomerNo')";
		...
		//Create the Account Payable
		 ...
		 VALUES(NULL, 6, '$datePurchaseBill', '$purchaseBillNo', 'PB', '$purchaseBillID', '$customerNameValue', 0, '$groundTotalRound')";
		...
		account4_documentType, account4_documentTypeID, account4_description, account4_debit, account4_credit) 
		VALUES(NULL, '$purchaseBillAccount3', '$datePurchaseBill', '$purchaseBillNo', 'PB', '$purchaseBillID', '$customerNameValue', '$groundTotalRound', 0)";
	...
	$sqlMaxID = "SELECT purchaseBill_no FROM tbpurchasebill ORDER BY purchaseBill_id DESC LIMIT 0,1";
	...
	if(mysqli_num_rows($resultMax) > 0){
		$rowLastPurchaseBillNo = mysqli_fetch_row($resultMax);
	...
	//get the duplicate Purchase Bill info only if idPurchaseBillDuplicate parameter set
	$duplicateStatus = 0;
	$customerID = 0;
	
	if(isset($_GET['idPurchaseBillDuplicate'])){
		$duplicateStatus = 1;
		$purchaseBillDuplicateID = $_GET['idPurchaseBillDuplicate'];
		...
		$sqlPurchaseBillInfo = "SELECT purchaseBill_content, purchaseBill_terms, purchaseBill_subTotal, purchaseBill_taxTotal, purchaseBill_grandTotal,
		purchaseBill_discountTotal, purchaseBill_totalAfterDiscount, purchaseBill_roundAmount, purchaseBill_grandTotalRound, purchaseBill_roundStatus 
		FROM tbpurchasebill WHERE purchaseBill_id = '$purchaseBillDuplicateID'";
		...
		WHERE purchaseBillDetail_purchaseBillID = '$purchaseBillDuplicateID' ORDER BY purchaseBillDetail_sortID ASC";
...
function check(it) {
	...
	function CheckPurchaseBillNo(){		
		var checkEmptyPurchaseOrderNo = $("#purchaseBillNo").val();
	...
	function GetDuplicateInfo(){
		var checkDuplicatePurchaseBillNo = $("#purchaseBillDuplicate").val();
		var customerID = $("#customerIDvalue").val();
		
		if(checkDuplicatePurchaseBillNo===""){
			//do nothing				
			purchaseBillDuplicate.focus();
		}else{
			
			var data = "purchaseBillNo="+$("#purchaseBillDuplicate").val();
			...
			$.ajax({			 
				...
				{
					if(r=="0"){
						$("#duplicateInfo").html("No Purchase Bill!");
						purchaseBillDuplicate.focus();
						...
						window.location.href = "createPurchaseBill.php?idCustomer="+customerIDvalue+"&idPurchaseBillDuplicate="+quotationIDvalue;
	...
	function ResubmitForm(g){
		if(g==0){					
			...
			var u = confirm("Do you want to Create Purchase Bill?");
			if(u==true){
				$('#purchaseBillForm').attr('onsubmit', 'return true');
...
<p></p>
<form id="purchaseBillForm" action="createPurchaseBill.php" method="POST" onsubmit="return CheckAndSubmit()">
<table width="1200px" cellpadding="0" border="1" class="mystyle">
<tr><td colspan="4" align="center"><h1>Create Purchase Bill</h1></td><td></td></tr>
...
</td><td><input type="text" name="purchaseBillDuplicate" id="purchaseBillDuplicate" size="12" maxlength="30">&nbsp;<img src="images/duplicatefinal.png" onclick="GetDuplicateInfo()"><span id="duplicateInfo" style="color:blue;font-size:12px"></span></td></tr>

<tr><td>To</td><td><input type="text" id="customerNameValue" name="customerNameValue" size="60" value="<?php if(($duplicateStatus==1)&&($customerID <> 0)){echo $rowCustomerInfo[0];}?>" readonly></td><td></td><td>Purchase Bill No</td><td><input type="text" name="purchaseBillNo" id="purchaseBillNo" size="12" maxlength="30" value="<?php echo $strLastPurchaseBillNo;?>" onchange="CheckPurchaseBillNo()" autofocus><span id="info"><img border="0" src="images/Terminate.png"></span></td></tr>
<tr>
	...
	?>
	<input id="demo2" type="text" name="purchaseBillDate" size="10" onchange="CheckAccountingPeriod()" value="<?php echo date('d/m/Y');?>" readonly>
	&nbsp;
</td>
</tr>
<tr><td>Address</td><td><input type="text" name="customerAddressValue" id="customerAddressValue" size="70" value="<?php if(($duplicateStatus==1)&&($customerID <> 0)){echo $rowCustomerInfo[4];}?>" readonly></td><td></td><td>From</td><td><input type="text" name="purchaseBillFrom" size="20" maxlength="50"></td></tr>
<tr><td>Telephone</td><td><input type="text" name="customerTelValue" id="customerTelValue" value="<?php if(($duplicateStatus==1)&&($customerID <> 0)){echo $rowCustomerInfo[1];}?>" readonly></td><td></td><td>Account</td>
<td bgcolor="eff5b8">
<?php
...
<tr><td>Email</td><td><input type="text" name="customerEmailValue" id="customerEmailValue" size="40" maxlength="100" value="<?php if(($duplicateStatus==1)&&($customerID <> 0)){echo $rowCustomerInfo[2];}?>"></td><td></td>
<td>Customer Invoice</td><td><input type="text" name="purchaseBillCustomerNo" id="purchaseBillCustomerNo" size="12" maxlength="80"></td></tr>

<tr><td colspan="5" align="left"></td></tr>
<tr><td colspan="5" align="left">&nbsp;<input type="text" name="purchaseBillTitle" maxlength="200" size="150" placeholder="Purchase Bill Title"></td></tr>
<tr><td colspan="5" align="left"><textarea name="purchaseBillContent" id="purchaseBillContent"><?php if($duplicateStatus==1){echo $rowPurchaseBillInfo[0];}?></textarea></td></tr>
</table>
...
<?php
	...
	if($duplicateStatus==1){		
		if(mysqli_num_rows($resultPurchaseBillDetailInfo) > 0){
			...
			while($rowPurchaseBillDetailInfo = mysqli_fetch_row($resultPurchaseBillDetailInfo)){
				...
				echo "<td>";
				if($rowPurchaseBillDetailInfo[10] == 1){
				...
				echo "</td>";
				echo "<td><textarea class=$style2name id='noCol1[]' name='noCol1[]' maxlength='6'>$rowPurchaseBillDetailInfo[0]</textarea></td>";
				
				//get a unique ID for name column				
				$idNamePHP = "id123456789".$intUniqueNameID;				
				
				
				//echo "<td><textarea class='styled' id='$idNamePHP' name='noCol2[]' maxlength='200' ondblclick='popupwindow()'>$rowPurchaseBillDetailInfo[1]</textarea></td>";
				
				
				if($rowPurchaseBillDetailInfo[10] == 1){			
					echo "<td><textarea class='styled' style=\"font-weight: bold; width: 480px;\" id='$idNamePHP' name='noCol2[]' maxlength='200' ondblclick='popupwindow()'>$rowPurchaseBillDetailInfo[1]</textarea></td>";
				}else{
					echo "<td><textarea class='styled' style='width: 480px;'  id='$idNamePHP' name='noCol2[]' maxlength='200' ondblclick='popupwindow()'>$rowPurchaseBillDetailInfo[1]</textarea></td>";
				}
				...
				if($rowPurchaseBillDetailInfo[2]==0.00){
					echo "<td><textarea class=$style3name id='noCol3[]' name='noCol3[]' maxlength='10' onblur='CalculateRowTotal()'></textarea></td>";
				}else{
					echo "<td><textarea class=$style3name id='noCol3[]' name='noCol3[]' maxlength='10' onblur='CalculateRowTotal()'>$rowPurchaseBillDetailInfo[2]</textarea></td>";
				}
				
				echo "<td><textarea class=$style3name id='noCol4[]' name='noCol4[]' maxlength='10'>$rowPurchaseBillDetailInfo[3]</textarea></td>";
				echo "<td><textarea class=$style3name id='noCol5[]' name='noCol5[]' maxlength='15' onblur='CalculateRowTotal()'>$rowPurchaseBillDetailInfo[4]</textarea></td>";
				echo "<td><textarea class=$style3nameTotal id='noCol6[]' name='noCol6[]' maxlength='15' readonly>$rowPurchaseBillDetailInfo[5]</textarea></td>";
				//discount amount
				if($rowPurchaseBillDetailInfo[12]==0.00){
					echo "<td><textarea class=$style3name id='noCol11[]' name='noCol11[]' maxlength='10' onblur='CalculateRowTotal()'></textarea></td>";
				}else{
					echo "<td><textarea class=$style3name id='noCol11[]' name='noCol11[]' maxlength='10' onblur='CalculateRowTotal()'>$rowPurchaseBillDetailInfo[12]</textarea></td>";
				}
				//discount amount
				echo "<td><textarea class=$style3nameTotal id='noCol12[]' name='noCol12[]' maxlength='15' readonly>$rowPurchaseBillDetailInfo[13]</textarea></td>";
				//total after discount
				echo "<td><textarea class=$style3nameTotal id='noCol13[]' name='noCol13[]' maxlength='15' readonly>$rowPurchaseBillDetailInfo[14]</textarea></td>";
				...
				if(mysqli_num_rows($resultGetTaxRate) > 0){
					$taxRateID = $rowPurchaseBillDetailInfo[6];
				...
				echo "<td><textarea class=$style3nameTotal id=$idNameValue2 name='noCol8[]' readonly>$rowPurchaseBillDetailInfo[7]</textarea></td>";
				echo "<td><textarea class=$style3nameTotal id='noCol9[]' name='noCol9[]' maxlength='15' readonly>$rowPurchaseBillDetailInfo[8]</textarea></td>";
				echo "<td><textarea class=$style3nameTotal id='noCol10[]' name='noCol10[]' maxlength='15' readonly>$rowPurchaseBillDetailInfo[9]</textarea></td>";

				$strPurchaseOrderCheckbox2 = $rowPurchaseBillDetailInfo[10];			
				if($numOfRows > 1){
					if($counter == 1){
...
<?php
	if($duplicateStatus==1){	
		if(mysqli_num_rows($resultPurchaseBillDetailInfo) > 0){
			...
			echo "<tr><td style='width:76%'></td><td style='width:15%'>Subtotal</td><td style='width:9%'><input type='text' id='subTotal' name='subTotal' class='styled4' value='$rowPurchaseBillInfo[2]' readonly></td><tr>";
			
			echo "<tr><td style='width:76%'></td><td style='width:15%'>Discount Total</td><td style='width:9%'><input type='text' id='discountTotal' name='discountTotal' class='styled4' value='$rowPurchaseBillInfo[5]' readonly></td><tr>";
			echo "<tr><td style='width:76%'></td><td style='width:15%'>Total After Discount</td><td style='width:9%'><input type='text' id='totalAfterDiscount' name='totalAfterDiscount' class='styled4' value='$rowPurchaseBillInfo[6]' readonly></td><tr>";
			
			echo "<tr><td style='width:76%'></td><td style='width:15%'>Tax</td><td style='width:9%'><input type='text' id='taxTotal' name='taxTotal' class='styled4' value='$rowPurchaseBillInfo[3]' readonly></td><tr>";
			echo "<tr><td style='width:76%'></td><td style='width:15%'>Total</td><td style='width:9%'><input type='text' id='grandTotal' name='grandTotal' class='styled4' value='$rowPurchaseBillInfo[4]' readonly></td><tr>";
			echo "<tr><td style='width:76%'></td><td style='width:15%'>Round</td><td style='width:9%'><input type='text' id='roundAmount' name='roundAmount' class='styled4' value='$rowPurchaseBillInfo[7]' readonly>";
			
			if($rowPurchaseBillInfo[9] == 1){
			...
			echo "</td><tr>";
			echo "<tr><td style='width:76%'></td><td style='width:15%'>Rounded Total</td style='width:9%'><td><input type='text' id='groundTotalRound' name='groundTotalRound' class='styled4' value='$rowPurchaseBillInfo[8]' readonly><input type='hidden' id='counterValue' value='$counter'></td><tr>";
			echo "</table>";
		}
	}
?>
...
<table width="1200px" cellpadding="0" border="1" class="mystyle">
<tr><td colspan="5" align="left"><textarea name="purchaseBillTerms" id="purchaseBillTerms"><?php if($duplicateStatus==1){echo $rowPurchaseBillInfo[1];}else{echo $rowCompanyName[1];} ?></textarea>
...
<p><input type="submit" id="bt" name="Submit" value="Create Purchase Bill" <?php if($duplicateStatus==1){echo "disabled";}?>></p>
