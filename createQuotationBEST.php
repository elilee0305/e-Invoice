<?php 	
	...
	if(isset($_POST['Submit'])){
		...
		quotation_subTotal, quotation_taxTotal, quotation_grandTotal) 
		...
		if(isset($_POST['noCol1'])){
			...
			for($i=0; $i < $countNo; $i++){
				...
				quotationDetail_quotationID, quotationDetail_bold, quotationDetail_sortID) 
				VALUES(NULL, '$noValue1', '$noValue2', '$noValue3', '$noValue4', '$noValue5', '$noValue6', '$noValue7', '$noValue8', 
				'$noValue9', '$noValue10', '$quotationID', '$noValueCheckbox', '$sortID')";
	...
	$sqlCustomerInfo = "SELECT customer_name, customer_tel, customer_email, customer_attention FROM tbcustomer WHERE 
...
<title>Quotation Trucking & Payroll System</title>
	...
	for (row = 1; row < myTab.rows.length; row++) {
		 for (c = 0; c < myTab.rows[row].cells.length; c++) {   // EACH CELL IN A ROW.
			if(c == 4){
				...
				if(isNaN(element.childNodes[0].value)){
					actualQty = 1;
				}else{
			...
			if(c == 6){
				...
				if(isNaN(element.childNodes[0].value)){		
					...
					if(element.childNodes[0].value==""){
						...
						var actualPrice = element.childNodes[0].value;
						rowTotal = (actualQty * actualPrice).toFixed(2);
						element2.childNodes[0].value = rowTotal;
			...
			if(c == 12){
				...
				if(element.childNodes[0].value=="0.00"){
					...
					element3.childNodes[0].value = rowTotal;
					grandTotalValue = grandTotalValue + parseFloat(rowTotal);
					
					rowRate = 0;
				}else{
					rowRate = element.childNodes[0].value;
					rowRateTotal = (rowTotal * rowRate)/100;
					//parseFloat function to change string to number
					rowGrandTotal = parseFloat(rowTotal) + parseFloat(rowRateTotal);
					element2.childNodes[0].value = rowRateTotal.toFixed(2);		
					element3.childNodes[0].value = rowGrandTotal.toFixed(2);
					taxTotalValue = taxTotalValue + rowRateTotal;
					grandTotalValue = grandTotalValue + rowGrandTotal;					
...
// ARRAY FOR HEADER.
    ...
    arrHead = ['', '', 'No', 'Description', 'Qty', 'Unit', '@Price', 'Total', 'Disc%', 'Disc Amt', 'Total after Disc', 'Tax Code', 'Rate', 'Tax Total', 'Grand Total'];      // SIMPLY ADD OR REMOVE VALUES IN THE ARRAY FOR TABLE HEADERS.
	...
	function createTableTotal() {
		...
		empTableTotal.setAttribute('width', '800px');
            ...
            if (c == 0) {           // FIRST COLUMN.
               ...
               // ele.setAttribute('value', '');				
				...
				if(c == 12){
					...
					var myTab = document.getElementById('empTable');					
					var element2 = myTab.rows.item(rowCnt).cells[9];
					element2.childNodes[0].value = "0.00";				
				...
				if(c == 13){
					...
					//default value inside textarea
					var myTab = document.getElementById('empTable');					
					var element2 = myTab.rows.item(rowCnt).cells[10];
					element2.childNodes[0].value = "0.00";
				...
				if(c == 14){
					...
					//default value inside textarea
					var myTab = document.getElementById('empTable');					
					var element2 = myTab.rows.item(rowCnt).cells[11];
					element2.childNodes[0].value = "0.00";
				}
...
<table width="830" cellpadding="0" border="1" class="mystyle">
<tr bgcolor="AFCCB0"><td colspan="5" align="center"><h1>Create Quotation</h1></td></tr>
<tr><td>Date</td><td><input id="demo2" type="text" name="quotationDate" size="10" readonly>&nbsp;<img src="images2/cal.gif" onclick="javascript:NewCssCal('demo2','ddMMyyyy')" style="cursor:pointer"/></td><td>&nbsp;</td><td>Our Reference No</td><td><input type="text" name="quotationNo" id="quotationNo" size="12" value="<?php echo $strLastQuotationNo;?>" onchange="CheckQuotationNo()"><span id="info"><img border="0" src="images/Terminate.png"></span></td></tr>
...
<tr><td>Attention</td><td><input type="text" name="quotationAttention" size="40" maxlength="100" value="<?php echo $resultCustomerInfo2[3];?>"></td><td></td><td>Tel</td><td></td></tr>
<tr><td>Telephone</td><td><?php echo $resultCustomerInfo2[1]?></td><td></td><td>Fax</td><td></td></tr>
<tr><td>Email</td><td><input type="text" name="quotationEmail" size="40" maxlength="100" value="<?php echo $resultCustomerInfo2[2];?>"></td><td></td><td>Mobile</td><td></td></tr>
...
<table width="830" cellpadding="0" border="1" class="mystyle">
<tr><td colspan="5" align="left"><textarea rows="20" cols="100" name="quotationTerms" maxlength="1000"><?php echo $resultCompanyName2[1]; ?></textarea></td></tr>
